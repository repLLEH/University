...
class LSTM(object):
	def __init__(self, input_size, hidden_size, init_range = 1.0, previous = None):
		...
		self.b_i, self.b_f, self.b_o, self.b_j = np.zeros(h), np.ones(h) * 3, np.zeros(h), np.zeros(h)

		self.dW_hi, self.dW_hf, self.dW_ho, self.dW_hj = np.zeros(h, h), np.zeros(h, h), np.zeros(h, h), np.zeros(h, h)
		self.dW_xi, self.dW_xf, self.dW_xo, self.dW_xj = np.zeros(h, n), np.zeros(h, n), np.zeros(h, n), np.zeros(h, n)
		self.db_i, self.db_f, self.db_o, self.db_j = np.zeros(h), np.zeros(h), np.zeros(h), np.zeros(h)
	...
	def initSequence(self):
		...
		if hasattr(self, 'previous'):
			...
			self.h[0] = np.zeros(self.hidden_size)
			self.c[0] = np.zeros(self.hidden_size)

		if hasattr(self, 'next'):
			self.dh_prev = self.next.dh_prev
			self.dc_prev = self.next.dc_prev
		else:
			self.dh_prev = np.zeros(self.hidden_size)
			self.dc_prev = np.zeros(self.hidden_size)
	...
	def forward(self, x_t):
		...
		self.cell_update[t] = np.tanh(np.dot(self.W_hj, h) + np.dot(self.W_xj, x_t) + self.b_j)

		self.c[t] = self.input_gate[t] * self.cell_update[t] + self.forget_gate[t] * self.c[t-1]
		self.ct[t] = np.tanh(self.c[t])
	...
	def backward(self, dh):
		...
		dh = dh + self.dh_prev
		dc = np.tanh_grad(self.ct[t]) * self.output_gate[t] * dh + self.dc_prev
		...
		d_update = np.tanh_grad(self.cell_update[t]) * self.input_gate[t] * dc
